...
- **无法匹配现有分类时的处理**：当一条评论在现有列表里没有合适分类时：
  - **评论提到了具体功能/特征，但分类目录里没有相关分类** → `99-XX 待分类: <建议名称>`，方便后续把这些建议补充进 `references/<app>.md`
  - **评论本身表述不清/含糊** → `12-10 表述不清暂无法归类`
- **99-XX 待分类的具体要求**：
  - 触发前提：评论必须明确提到了**具体功能或特征**（如某项操作、某个界面、某种文件格式、某种使用场景等）；只是笼统说好/不好、没提任何具体功能的评论**不**触发 99-XX
  - 编号规则：`XX` 从 `01` 起递增；**同一类问题的多条评论必须使用同一个 99-XX 编号**（例如多条都在反馈"裁剪后无法分享"，统一用 `99-01 待分类: 裁剪后无法分享`），不同类问题用不同编号（`99-02`、`99-03` …）
  - 建议名称要求：简洁、可复用、能直接当分类项用，**不要照搬评论原文**（例：写 `裁剪后文件无法分享`，而不是 `i can't share my cuts after trimming`）
  - 不要滥用：能直接归入现有具体分类的评论、笼统好评/差评（未提及具体功能特征的），都不应使用 99-XX
  - 多分类拆行时，99-XX 也可作为其中一行的分类（与其他分类同等处理，仍按多分类规则染色 `#CAECFA`）
  - 分类完成后，统计输出中需单独列出本次新增的所有 99-XX 编号及建议名称、对应条数，方便用户回顾
